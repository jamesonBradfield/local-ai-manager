...
class SteamWatcher:
    ...
    def _switch_agent(self, agent_type: str) -> None:
        ...
        config_path = self.config.opencode.config_dir / self.config.opencode.config_file
    ...
    @staticmethod
    def find_watcher_processes() -> list[psutil.Process]:
        """Find all running Steam watcher processes.

        Returns:
            List of psutil.Process objects for watcher processes.
        """
        processes = []
        for proc in psutil.process_iter(["name", "cmdline"]):
            try:
                ...
                if cmdline_list:
                    ...
                    if "steam" in cmdline.lower() and "watcher" in cmdline.lower():
    ...
    @staticmethod
    def is_running() -> bool:
        """Check if any Steam watcher process is running.

        Returns:
            True if at least one watcher process is found.
        """
        return len(SteamWatcher.find_watcher_processes()) > 0

    @staticmethod
    def stop_all() -> int:
        """Stop all Steam watcher processes.

        Returns:
            Number of processes terminated.
        """
        stopped = 0
        for proc in SteamWatcher.find_watcher_processes():
            try:
                proc.terminate()
                stopped += 1
            except (psutil.NoSuchProcess, psutil.AccessDenied):
